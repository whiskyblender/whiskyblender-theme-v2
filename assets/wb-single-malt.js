...
(function () {
  ...
  function init() {
    ...
    /* Which products' previews may switch to the New taller template. As of
       2026-09-07 only CUSTOM BLEND is moving to taller labels — single malt/cask
       are NOT getting them yet, so their preview stays Classic and the toggle
       hotspot isn't shown on them. The New malt/cask code paths below are kept
       intact (dark tone artwork, "Distilled at", cask svg); to switch malt/cask
       on later, just add 'singlemalt' / 'singlecask' here. 500ml only. */
    var NEW_TPL_PRODUCTS = ['customblend'];
    var newTplEligible = !is200ml && NEW_TPL_PRODUCTS.indexOf(productSlug) !== -1;
    /* Effective New state for THIS product: the persisted toggle AND eligibility.
       On an ineligible product a stale localStorage 'new' is inert. */
    function newActive() { return isNewTpl && newTplEligible; }
    function prevDims() { return newActive() ? PREVIEW_D_NEW : PREVIEW_D; }
    ...
    function initPreview() {
      ...
      /* Mirrors the close button's icon-then-label structure. Uses the theme's
         own assets/icon-eye.svg paths (previously unused) so it matches the
         rest of the icon set rather than an emoji, which would render in
         colour next to the close button's plain glyph. */
      var openBtnIcon = document.createElement('span');
      openBtnIcon.className = 'wbp-open-btn__icon';
      openBtnIcon.setAttribute('aria-hidden', 'true');
      openBtnIcon.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" width="16" height="16" fill="currentColor" stroke="currentColor" stroke-width="0.6" stroke-linejoin="round" aria-hidden="true"><path d="M9.524 4.8c-2.761.13-5.44 1.82-8.352 4.865a.484.484 0 0 0 0 .67c2.911 3.044 5.59 4.736 8.352 4.865 2.75.13 5.743-1.286 9.298-4.86a.48.48 0 0 0 0-.68c-3.555-3.574-6.549-4.99-9.298-4.86m-.049-1.04c3.177-.15 6.44 1.503 10.081 5.164a1.526 1.526 0 0 1 0 2.152c-3.641 3.66-6.904 5.314-10.08 5.165-3.167-.149-6.085-2.08-9.053-5.185a1.53 1.53 0 0 1 0-2.112C3.391 5.84 6.31 3.907 9.475 3.76"/><path d="M13.58 10a3.564 3.564 0 0 1-3.568 3.559A3.564 3.564 0 0 1 6.443 10a3.564 3.564 0 0 1 3.569-3.558A3.564 3.564 0 0 1 13.581 10m-3.568 2.517A2.524 2.524 0 0 0 12.542 10a2.524 2.524 0 0 0-2.53-2.517A2.524 2.524 0 0 0 7.482 10a2.524 2.524 0 0 0 2.53 2.517"/></svg>';
      openBtn.appendChild(openBtnIcon);
      openBtn.appendChild(document.createTextNode('Show label preview'));
